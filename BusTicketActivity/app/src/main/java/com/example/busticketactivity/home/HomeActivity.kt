            ...
            FireBaseRepo().getUser(email)
                .addOnCompleteListener {
                    if (it.isSuccessful) {
                        ...
                        val user = getPreferences( Context.MODE_PRIVATE).edit()

                        val data = Gson()//inisialisasi GSOn
//                        user.putString("dataUser", .apply()//menset data user sebagai shared preferences
                        val pref=getSharedPreferences("data",Context.MODE_PRIVATE)
                        val coba=pref.getString("dataUser","")
                        val coba2=data.fromJson(coba,UserObject::class.java)
                        Toast.makeText(this, "$data.toJson(list))", Toast.LENGTH_SHORT).show()
                        if (list != null) {
